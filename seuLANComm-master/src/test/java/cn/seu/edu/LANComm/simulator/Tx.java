...
public class Tx implements Runnable{
    ...
    private static volatile boolean paraReceived = true;
    private static final String localMAC = "00:0C:29:1E:AA:3F".trim();
                        ...
                        public void receivePacket(Packet packet) {
                            ...
                            if (Short.parseShort(DataLinkParameterEnum.FRAME_TYPE.getDataType()) == ethernetPacket.frametype) {
                                ...
                                if (decoder.getParameterIDentifier().getDataType().equals(DataLinkParameterEnum.PARAMETER_SETTING.getDataType())) {
                                    ...
                                    System.out.println("数据类型：" + DataLinkParameterEnum.PARAMETER_SETTING);
                    ...
                    if (paraReceived) {
                        ...
                        captor.loopPacket(-1, new PacketReceiver() {
                            ...
                            public void receivePacket(Packet packet) {
                                ...
                                if (Short.parseShort(DataLinkParameterEnum.FRAME_TYPE.getDataType()) == ethernetPacket.frametype) {
                                    FramingDecoder decoder = new FramingDecoder(packet.data);
                                    if (decoder.getParameterIDentifier().getDataType().equals(DataLinkParameterEnum.COMMUNICATION_START.getDataType())) {
                                        System.out.println("帧类型：" + ethernetPacket.frametype);
                                        System.out.println("目的MAC：" + MACStringConvertor.macToString(ethernetPacket.dst_mac));
                                        System.out.println("源MAC：" + MACStringConvertor.macToString(ethernetPacket.src_mac));
                                        System.out.println("数据类型：" + DataLinkParameterEnum.COMMUNICATION_START);
                                        stopSend = false;
                                        captor.breakLoop();
                                    }
                                }
    ...
    @SuppressWarnings("all")
    public void process(Packet packet, DataLinkParameterEnum parameterEnum) {
        EthernetPacket ethernetPacket = (EthernetPacket) packet.datalink;
        if (Short.parseShort(DataLinkParameterEnum.FRAME_TYPE.getDataType()) == ethernetPacket.frametype) {
            System.out.println("帧类型：" + ethernetPacket.frametype);
            System.out.println("目的MAC：" + MACStringConvertor.macToString(ethernetPacket.dst_mac));
    ...
    @SuppressWarnings("all")
    private static void showArray(float[] data) {
        if (data == null) {
    ...
    @SuppressWarnings("all")
    private static void showParaSetted(float[] data, CommunicationModeEnum modeEnum) {
    ...
    @SuppressWarnings("all")
    private static CommunicationModeEnum getModeEnum(int modeCode) {
